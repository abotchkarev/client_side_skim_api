h1. !http://railsapps.github.io/images/rails-36x36.jpg(Rails Bootstrap)! Rails, Bootstrap and Skim (Slim based CoffeScript templates)


Rails 4.2 example application that integrates Rails, Bootstrap 3.3 and Skim.
...
"Skim":https://github.com/appjudo/skim Fat-free client-side templates with Slim and CoffeeScript

h2. How to start this app

Make sure you have access to mysql DB via localhost:3306

$ cp .env .env.local

Populate .env.local with credentals (DB_USERNAME, DB_PASSWORD) to access your DB
Default DB names to be created are 'railsapp_development' and  'railsapp_test'
Edit database.yml for changes

$ rake db:setup
$ rails s
 
h2. Text below comes from app generator
